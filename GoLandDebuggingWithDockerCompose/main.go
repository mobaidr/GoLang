...
func getDBConnection(ctx context.Context) *pgxpool.Pool {
	...
	//const connectionString = "postgres://goland:goland@%s:5435/goland?sslmode=disable"

	const connectionString = "postgres://obaid:obaid@%s:5435/obaid?sslmode=disable"

	var dbConnectionString = fmt.Sprintf(connectionString, host)

	log.Printf("Connection String : %s", dbConnectionString)
